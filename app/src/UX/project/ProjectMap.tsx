...
import ProjectItemNode from "./projectNavigation/ProjectItemNode";
...
class ProjectMap extends Component<IProjectMapProps, IProjectMapState> {
  ...
  */
  ...
  async _generateContext() {
    ...
    // First pass: add root items that have children (relationship hierarchies)
    ...
    // Second pass: if no items were added, show standalone "important" items
    // (entity types, block types, item types) even without relationships
    if (context.nodes.length === 0) {
      for (const projectItem of projectItemsList) {
        if (projectItem.projectPath && projectItem.parentItemCount === 0) {
          const itemType = projectItem.itemType;
          // Show primary content types even without relationships
          if (
