...
import { ensureCreatorToolsReady } from "./CreatorToolsReady";
import { getModeFromString, AppMode } from "./AppMode";
...
interface AppState {
  ...
  worldUrl?: string;
  // For worldViewer mode - which 2D/3D layout to show. When set, the standalone
  // worldViewer route mounts the full WorldDisplay (map + 3D + side-by-side)
  // instead of the 3D-only WorldViewer. Parsed from the `view` query param.
  ...
  cameraY?: number;
  cameraZ?: number;
  // For worldViewer mode - lowest world Y to render. Used by the Y-floor
  // regression test to force a high floor and confirm low content is culled.
  minRenderY?: number;
}

interface UrlParseSideEffects {
  ensureProjectFromGalleryId?: string;
  inputPath?: string;
  createLegacyProject?: {
    mode: AppMode;
    selectedItem: string;
...
interface ParsedUrlStateResult {
  state?: AppState;
  sideEffects: UrlParseSideEffects;
}
...
export default class AppMain extends Component<AppProps, AppState> {
  ...
  public async _loadLocalStorageProject() {
    ...
    if (newProject) {
      const mode: AppMode = getModeFromString(CreatorToolsHost.initialMode) ?? AppMode.home;
  ...
  private _handleReturnHomeFromNotFound = () => {
    try {
      if (typeof window !== "undefined" && window.history && window.history.replaceState) {
        window.history.replaceState(null, "", AppRouter.getHomeNavigationPath());
      }
    } catch {
  ...
  private async loadCreatorTools(instance: CreatorTools) {
    ...
    if (newState) {
      ...
      const modeFromInitial = getModeFromString(CreatorToolsHost.initialMode);
      if (modeFromInitial) {
  ...
  private _getFirstQueryValue(queryVals: { [path: string]: string }, keys: string[]): string | undefined {
    for (const key of keys) {
      const value = queryVals[key];
      if (value !== undefined) {
        return value;
      }
    }
  ...
  private _isTruthyQueryValue(raw: string | undefined): boolean {
    return raw === "true" || raw === "1" || raw === "" || raw === "yes";
  }

  /**
   * Maps the `view` query param to a WorldViewMode for the standalone
   * worldViewer route. Returns undefined for an empty/unknown value, which
   * keeps the legacy 3D-only WorldViewer as the default.
   *   map               -> mapOnly (2D map only)
   *   3d / 3donly        -> fullWorldView (3D only, via WorldDisplay)
   *   split / 3dmapright -> worldViewer3D (3D left, map right)
   *   mapleft / 3dmapleft-> worldViewerMapOnLeft (map left, 3D right)
   */
  private static _parseWorldViewMode(value: string): WorldViewMode | undefined {
    switch (value) {
      case "map":
      case "maponly":
      case "2d":
        return WorldViewMode.mapOnly;
      case "3d":
      case "3donly":
      case "full":
        return WorldViewMode.fullWorldView;
      case "split":
      case "3dmap":
      case "3dmapright":
      case "mapright":
        return WorldViewMode.worldViewer3D;
      case "mapleft":
      case "3dmapleft":
        return WorldViewMode.worldViewerMapOnLeft;
      default:
        return undefined;
    }
  }

  private _applyUrlParseSideEffects(sideEffects: UrlParseSideEffects) {
    if (sideEffects.ensureProjectFromGalleryId) {
      this._ensureProjectFromGalleryId(sideEffects.ensureProjectFromGalleryId);
    }

    if (sideEffects.inputPath !== undefined) {
      const rootAndFocus = StorageUtilities.getRootAndFocusPathFromInputPath(decodeURIComponent(sideEffects.inputPath));
      CreatorToolsHost.projectPath = rootAndFocus.basePath;
      CreatorToolsHost.focusPath = rootAndFocus.focusPath;
    }

    if (sideEffects.createLegacyProject) {
  ...
  private _getStateFromUrlWithSideEffects(dontProcessQueryStrings?: boolean): AppState | undefined {
    const parsed = this._parseStateFromUrl(dontProcessQueryStrings);
    this._applyUrlParseSideEffects(parsed.sideEffects);
    return parsed.state;
  }

  private _parseStateFromUrl(dontProcessQueryStrings?: boolean): ParsedUrlStateResult {
    const sideEffects: UrlParseSideEffects = {};
      ...
      if (firstSlash > 1) {
        if (queryVals["updates"] !== undefined) {
          ...
          if (openToken === "gp") {
            ...
            sideEffects.ensureProjectFromGalleryId = openData;
          }
        }
      }
    }
    ...
    if (!dontProcessQueryStrings && queryVals["input"] !== undefined) {
      sideEffects.inputPath = queryVals["input"];
    }

    // Handle ?mode= query parameter for direct navigation
    if (!dontProcessQueryStrings && queryVals["mode"] !== undefined) {
      const modeFromQuery = getModeFromString(queryVals["mode"].toLowerCase());
      if (modeFromQuery) {
        // Decode geometry and texture URLs since they come URL-encoded from query params
        const geometryUrlRaw = this._getFirstQueryValue(queryVals, ["geometry", "geo"]);
        const textureUrlRaw = this._getFirstQueryValue(queryVals, ["texture", "tex"]);
        // Parse skipVanilla parameter (defaults to true if not specified)
        const skipVanillaRaw = this._getFirstQueryValue(queryVals, ["skipvanilla", "skipVanilla"]);
        ...
        const contentRootRaw = this._getFirstQueryValue(queryVals, ["contentroot", "contentRoot"]);
        if (contentRootRaw) {
        ...
        const cameraX = this._getFirstQueryValue(queryVals, ["camerax", "cameraX"]);
        const cameraY = this._getFirstQueryValue(queryVals, ["cameray", "cameraY"]);
        const cameraZ = this._getFirstQueryValue(queryVals, ["cameraz", "cameraZ"]);

        // Parse structure URL for structureViewer mode
        const structureUrlRaw = this._getFirstQueryValue(queryVals, ["structure", "struct"]);

        // Parse world URL for worldViewer mode
        const worldUrlRaw = this._getFirstQueryValue(queryVals, ["world", "worldurl"]);

        // Parse the world view layout for worldViewer mode. When present, the
        // standalone worldViewer route mounts the full WorldDisplay so the 2D
        // map and side-by-side layouts are reachable (and testable) by URL.
        const worldViewRaw = (this._getFirstQueryValue(queryVals, ["view", "worldview"]) || "").toLowerCase();
        const worldViewMode = AppMain._parseWorldViewMode(worldViewRaw);

        // Parse minRenderY for worldViewer mode (Y-floor regression test hook)
        const minRenderYRaw = this._getFirstQueryValue(queryVals, ["minrendery", "minRenderY"]);

        // Parse hideChrome parameter for headless rendering (hides all UI controls)
        const hideChromeRaw = this._getFirstQueryValue(queryVals, ["hidechrome", "hideChrome"]);
        const hideChrome = this._isTruthyQueryValue(hideChromeRaw);

        // Parse showEditorTools parameter to enable VolumeEditor toolbar (brush, pencil, selection)
        const showEditorToolsRaw = this._getFirstQueryValue(queryVals, ["showeditortools", "showEditorTools"]);
        const showEditorTools = this._isTruthyQueryValue(showEditorToolsRaw);

        return {
          state: {
            mode: modeFromQuery,
            activeProject: null,
            selectedItem: this._getFirstQueryValue(queryVals, ["block", "item", "mob"]) || this.state?.selectedItem,
            ...
            minRenderY: minRenderYRaw !== undefined ? parseFloat(minRenderYRaw) : undefined,
          },
          sideEffects,
        };
      }
    ...
    if (hash === "" && !this._lastHashProcessed) {
      this._lastHashProcessed = hash;
      return { state: undefined, sideEffects };
    }
    ...
    if (hash !== this._lastHashProcessed) {
      ...
      if (firstSlash > 1) {
        ...
        if (commandToken === "project") {
          ...
          if (segments.length >= 1) {
            ...
            if (existingProject) {
              ...
              sideEffects.createLegacyProject = {
                mode: AppMode.project,
        ...
        const commandMode = getModeFromString(commandToken);
  ...
  componentDidMount() {
    if (!this._isMountedInternal) {
      ...
      ensureCreatorToolsReady()
        .then((creatorTools) => this.loadCreatorTools(creatorTools))
        .then(this.processInitialUrl)
        .catch((err) => {
          const message = err instanceof Error ? err.message : String(err);
          Log.error("Failed to initialize CreatorTools in AppMain: " + message);
          this.setHomeWithError("Failed to initialize CreatorTools.");
        });
  ...
  processInitialUrl() {
    ...
    // Check if the pathname is valid SPA route. If not, show 404.
    let initialAppMode = AppMode.home;

    if (stateFromUrl) {
      initialAppMode = stateFromUrl.mode;
    } else if (CreatorToolsHost.initialMode) {
      const mode = getModeFromString(CreatorToolsHost.initialMode);
    ...
    this.setState((prevState) => ({
      ...
      worldViewMode: stateFromUrl?.worldViewMode,
      ...
      // Vertical render floor for the Y-floor regression test hook
      minRenderY: stateFromUrl?.minRenderY,
    }));
  }
  ...
  private async _newProjectFromGitHubTemplate(
    ...
    if (gitHubOwner !== undefined && gitHubRepoName !== undefined) {
      ...
      try {
        ...
        const errorMessage = "Could not save the new project. " + (e?.message ?? e?.toString());

        telemetryService.trackException({
          exception: e instanceof Error ? e : new Error(errorMessage),
          properties: {
            [TelemetryProperties.ERROR_MESSAGE]: errorMessage,
            [TelemetryProperties.LOCATION]: "App._newProjectFromGitHubTemplate.save",
            [TelemetryProperties.ACTION_TYPE]: "projectCreation",
          },
          severityLevel: TelemetrySeverity.ERROR,
        });
  ...
  render() {
    ...
    if (ct === undefined || !ct.isLoaded) {
      ...
      if (this.state.worldViewMode !== undefined) {
        // Standalone full world display - loads .mcworld from URL and mounts the
        // WorldDisplay (2D map, 3D, and side-by-side layouts) per the `view` param.
        interior = (
          <LazyStandaloneWorldDisplay
            heightOffset={heightOffset}
            key={`app-worlddisplay-${this.state.worldViewMode}`}
            worldUrl={this.state.worldUrl}
            initialDisplayMode={this.state.worldViewMode}
            hideChrome={this.state.hideChrome}
            cameraX={this.state.cameraX}
            cameraY={this.state.cameraY}
            cameraZ={this.state.cameraZ}
        ...
        interior = (
          <LazyWorldViewer
            ...
            cameraY={this.state.cameraY}
            cameraZ={this.state.cameraZ}
            minRenderY={this.state.minRenderY}
          />
        );
      }
