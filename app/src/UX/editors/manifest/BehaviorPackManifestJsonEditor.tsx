...
> {
  ...
  _handleItemSelected(elt: any, event: { selectedIndex?: number } | undefined) {
    if (
      event === undefined ||
  ...
  render() {
    ...
    if (def && def.header) {
      behaviorPackTitle = def.header.name || behaviorPackTitle;
      behaviorPackUuid = def.header.uuid;
    }
    ...
    return (
      ...
      >
        <div className="bpme-header">
          <div className="bpme-header-title">{behaviorPackTitle}</div>
          <div className="bpme-header-subtitle">{behaviorPackSubtitle}</div>
          {behaviorPackUuid && (
            <div
