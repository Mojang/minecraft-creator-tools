...
import IPersistable from "../../UX/types/IPersistable";
...
export default class MCWorldEditor extends Component<IMCWorldEditorProps, IMCWorldEditorState> {
  ...
  private async _updateManagerOnFolder() {
    if (this.state !== undefined && this.state.folderToEdit !== undefined) {
      if (this.state.folderToEdit !== this._lastFolderEdited) {
        ...
        await Database.ensureFormLoaded("world", "mcworld_properties");
      }
    }
  }

  private async _updateManagerOnFile() {
    if (this.state !== undefined && this.state.fileToEdit !== undefined) {
      if (this.state.fileToEdit !== this._lastFileEdited && this.state.fileToEdit) {
        this._lastFileEdited = this.state.fileToEdit;

        MCWorld.ensureOnFile(this.state.fileToEdit, this.props.project, this._handleMcworldLoaded);
  ...
  render() {
    ...
    if (this.props.displayProps) {
      const formDef = Database.getForm("world", "mcworld_properties");
