...
export class ConsoleLogger implements ILogger {
  ...
  constructor(verbose = false, quiet = false, debug = false) {
  ...
  success(message: string): void {
    // Success messages always show (they're typically final results)
 ...
 */
export function createLogger(verbose = false, quiet = false, debug = false, silent = false): ILogger {
  if (silent) {
    return new SilentLogger();
  }
  return new ConsoleLogger(verbose, quiet, debug);
}
