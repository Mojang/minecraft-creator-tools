  ...
  async generate(project: Project, contentIndex: ContentIndex): Promise<ProjectInfoItem[]> {
    ...
    for (let i = 0; i < itemsCopy.length; i++) {
      ...
      if (pi.storageType === ProjectItemStorageType.singleFile) {
        // Wrap per-item work in try/catch so a single bad item can't wedge the entire
        // validation pass. If load/parse throws (or rejects), we log and continue.
        try {
          ...
          Log.error(
            `[LINESIZE] Error processing item index=${i} type=${pi.itemType} path=${pi.projectPath ?? "(none)"}: ${
              e && e.stack ? e.stack : e
            }`
          );
        }
