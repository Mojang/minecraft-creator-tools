  ...
  async generate(project: Project, contentIndex: ContentIndex): Promise<ProjectInfoItem[]> {
    ...
    for (const projectItem of itemsCopy) {
      if (projectItem.itemType === ProjectItemType.blocksCatalogResourceJson) {
        ...
        if (projectItem.file) {
          const blockCat = await BlocksCatalogDefinition.ensureOnFile(projectItem.file);
        ...
        if (projectItem.file) {
          const blockCat = await TerrainTextureCatalogDefinition.ensureOnFile(projectItem.file);
        ...
        if (projectItem.file) {
          const itemCat = await ItemTextureCatalogDefinition.ensureOnFile(projectItem.file);
