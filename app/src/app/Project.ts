...
export default class Project {
  ...
  public async addBrowserFile(path: string, file: File) {
    ...
    if (
      ...
      const operId = await this.#carto.notifyOperationStarted("Saving new JSON file '" + file.name + "'");

      const jsonContentStr = await file.text();

      await ProjectUtilities.ensureJsonItem(this, jsonContentStr, fileName);

      await this.#carto.notifyOperationEnded(operId, "New JSON file '" + file.name + "' added");
    }
  }
  ...
  ensureStoragePathIsCollapsed(storagePath: string) {
    for (const pathCollasped of this.collapsedStoragePaths) {
      if (pathCollasped === storagePath) {
        return;
      }
    }
  ...
  public getItemByFile(file: IFile) {
    let projectItem = this.getItemByStoragePath(file.storageRelativePath);

    if (projectItem !== undefined) {
      return projectItem;
    }

    return this.getItemByExtendedOrStoragePath(file.extendedPath);
  }
  ...
  public getItemByExtendedOrStoragePath(storagePath: string): ProjectItem | undefined {
    ...
    if (nextSlash > 1) {
      result = this.#itemsByStoragePath[path.substring(nextSlash)];
  ...
  async ensureProjectFolder(force?: boolean): Promise<IFolder> {
    ...
    if (
      ...
      if (folderExists) {
        ...
        this.#projectFolder = this.#carto.projectsStorage.rootFolder.ensureFolder(
      ...
      this.#projectFolder = this.#carto.projectsStorage.rootFolder.ensureFolder(this.name);
  ...
  async ensurePackByFolder(folder: IFolder, packType: PackType, isInWorld?: boolean) {
    ...
    const folderProjectItem = this.ensureItemByStoragePath(
