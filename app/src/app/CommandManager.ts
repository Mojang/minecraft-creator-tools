...
export default class CommandManager {
  static async runStandardCommand(carto: Carto, commandNumber: number) {
    ...
    const operId = carto.notifyOperationStarted("Running command " + commandName);

    if (cartoCommand.text !== undefined) {
      await CommandManager.runCommandText(carto, cartoCommand.text);
    } else {
      // alert("This command is not defined, yet.");
    }

    carto.notifyOperationEnded(operId, "Command " + commandName + " complete.");
  }
  ...
  static async runCommandList(carto: Carto, commandItems: string[]) {
    ...
    for (let i = 0; i < commandItems.length; i++) {
      ...
      if (commandText.length > 4 && !commandText.startsWith("#")) {
        ...
        const result = await carto.runMinecraftCommand(commandText);
