...
export default class MessageProxy {
  ...
  static init() {
    CreatorToolsHost.onMessage.subscribe(MessageProxy._handleNewMessage);
  }
  ...
  static send(commandName: MessageProxyCommands | string, channelId: string, data: any) {
    ...
    try {
      CreatorToolsHost.postMessage({ command: commandName, id: channelId, data: data });
    } catch (e) {
      let dataStr = data;
  ...
  static sendAsync(commandName: MessageProxyCommands | string, channelId: string, data: any): Promise<any> {
    if (CreatorToolsHost.postMessage === undefined) {
    ...
    CreatorToolsHost.postMessage({ command: commandStr, id: channelId, data: data });
  ...
  private static _handleNewMessage(source: any, message: any) {
    if (message.command && message.command.length > 0) {
      if (message.command.startsWith("async")) {
        const argSplit = Utilities.splitUntil(message.command, "|", 1);
